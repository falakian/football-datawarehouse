CREATE OR ALTER PROCEDURE Fact_Clubs_Daily_update
as
begin
  ...
  set @factCount = (select count(*) from fact.FactClubsDaily);
  
  set @tempCount = (select count(*) from tmp.FactClubsDaily);
  ...
  insert into logg.logg values('truncate','FactClubsDaily_tmp',@number,@startdate,getdate(), 'truncate table by Fact_Clubs_Daily_update');

  SET @number = (SELECT COUNT(*) FROM tmp.FactClubsDaily);
  SET @startdate = GETDATE();
  truncate table tmp.FactClubsDaily;
  insert into logg.logg values('truncate','FactClubsDaily',@number,@startdate,getdate(), 'truncate table by Fact_Clubs_Daily_update');
  ...
  from (select [competition_key],
				...
				[homePlays],
				[goalCount],
				RANK() OVER (PARTITION BY club_key ORDER BY time_key DESC) as rank_time
		from fact.FactClubsDaily) as res
  where res.rank_time = 1;
  SET @number = @@ROWCOUNT;
  insert into logg.logg values('insert','FactClubsDaily_tmp',@number,@startdate,getdate(), 'insert to table from fact.FactClubsDaily');
  ...
  select [competition_key],
		 ...
		 [homePlays],
		 [goalCount]
  from fact.FactClubsDaily
  SET @number = @@ROWCOUNT;
  insert into logg.logg values('insert','FactClubsDaily',@number,@startdate,getdate(), 'insert to table from fact.FactClubsDaily');
  ...
  while @currdate < CAST('2020-08-20' as date) begin
	...
	insert into logg.logg values('truncate','FactClubsDaily_source_home',@number,@startdate,getdate(), 'truncate table by Fact_Clubs_Daily_update');

	SET @number = (SELECT COUNT(*) FROM tmp.FactClubsDaily_source_away);
	SET @startdate = GETDATE();
	truncate table tmp.FactClubsDaily_source_away;
	insert into logg.logg values('truncate','FactClubsDaily_source_away',@number,@startdate,getdate(), 'truncate table by Fact_Clubs_Daily_update');

	SET @number = (SELECT COUNT(*) FROM tmp.FactClubsDaily_source_home_join);
	SET @startdate = GETDATE();
	truncate table tmp.FactClubsDaily_source_home_join;
	insert into logg.logg values('truncate','FactClubsDaily_source_home_join',@number,@startdate,getdate(), 'truncate table by Fact_Clubs_Daily_update');

	SET @number = (SELECT COUNT(*) FROM tmp.FactClubsDaily_source_away_join);
	SET @startdate = GETDATE();
	truncate table tmp.FactClubsDaily_source_away_join;
	insert into logg.logg values('truncate','FactClubsDaily_source_away_join',@number,@startdate,getdate(), 'truncate table by Fact_Clubs_Daily_update');

	SET @number = (SELECT COUNT(*) FROM tmp.FactClubsDaily_final);
	SET @startdate = GETDATE();
    truncate table tmp.FactClubsDaily_final;
	insert into logg.logg values('truncate','FactClubsDaily_final',@number,@startdate,getdate(), 'truncate table by Fact_Clubs_Daily_update');

	SET @number = (SELECT COUNT(*) FROM tmp.FactClubsDaily_final_tmp);
	SET @startdate = GETDATE();
	truncate table tmp.FactClubsDaily_final_tmp;
	insert into logg.logg values('truncate','FactClubsDaily_final_tmp',@number,@startdate,getdate(), 'truncate table by Fact_Clubs_Daily_update');
	...
	SET @startdate = GETDATE();
	insert into tmp.FactClubsDaily_final
    select isnull(fs.competition_key, ft.competition_key),
      isnull(fs.time_key,CONVERT(INT, FORMAT(@currdate, 'yyyyMMdd'))),
      isnull(fs.club_key, ft.club_key),
	  isnull(fs.winCount, 0) + isnull(ft.winCount, 0),
	...
	SET @number = @@ROWCOUNT;
	insert into logg.logg values('insert','FactClubsDaily_final',@number,@startdate,getdate(), 'insert to table from tmp.FactClubsDaily_source_away_join and tmp.FactClubsDaily_final_tmp');
	...
	select [competition_key],
		   ...
		   [homePlays],
		   [goalCount]
	from tmp.FactClubsDaily_final;
	SET @number = @@ROWCOUNT;
	insert into logg.logg values('insert','FactClubsDaily',@number,@startdate,getdate(), 'insert to table from tmp.FactClubsDaily_final');
    ...
    from tmp.FactClubsDaily_final;
	SET @number = @@ROWCOUNT;
	insert into logg.logg values('insert','FactClubsDaily_tmp',@number,@startdate,getdate(), 'insert to table from tmp.FactClubsDaily_final');
