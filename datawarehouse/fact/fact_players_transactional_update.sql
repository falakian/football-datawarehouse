CREATE OR ALTER PROCEDURE fact_players_transactional_update AS
BEGIN
    ...
    SET @factCount = (SELECT COUNT(*) FROM fact.FactPlayersTransactional);

    -- Get the count of records in the temporary table
    SET @tempCount = (SELECT COUNT(*) FROM tmp.FactPlayersTransactional_final);
    ...
    ELSE
        SET @currdate = (SELECT MAX(CONVERT(DATE, CONVERT(CHAR(8), f.[time_key]))) 
                         FROM fact.FactPlayersTransactional AS f);

    -- Increment the current date by one day
    SET @currdate = DATEADD(DAY, 1, @currdate);

    -- Truncate the temporary final table
	SET @number = (SELECT COUNT(*) FROM tmp.FactPlayersTransactional_final);
	SET @startdate = GETDATE();
    TRUNCATE TABLE tmp.FactPlayersTransactional_final;
    INSERT INTO logg.logg VALUES('Truncate', 'tmpFactPlayersTransactional_final',@number,@startdate, GETDATE(), 'truncate table by tmpFactPlayersTransactional_final');
    ...
    BEGIN
        ...
        INSERT INTO logg.logg VALUES('Truncate', 'tmpFactPlayersTransactional_source',@number,@startdate, GETDATE(), 'truncate table by FactPlayersTransactional_source');

		SET @number = (SELECT COUNT(*) FROM tmp.FactPlayersTransactional_join);
		SET @startdate = GETDATE();
        TRUNCATE TABLE tmp.FactPlayersTransactional_join;
        INSERT INTO logg.logg VALUES('Truncate', 'tmpFactPlayersTransactional_join',@number,@startdate, GETDATE(), 'truncate table by FactPlayersTransactional_join');
		...
		SET @startdate = GETDATE();
        INSERT INTO tmp.FactPlayersTransactional_final
            SELECT [player_key], [time_key], [competition_key],
                   [club_key], [game_key], [type], [minute]
            FROM tmp.FactPlayersTransactional_join;
		SET @number = @@ROWCOUNT;
        INSERT INTO logg.logg VALUES('Insert', 'tmpFactPlayersTransactional_final',@number,@startdate, GETDATE(), 'insert to table from tmpFactPlayersTransactional_join');
    ...
    INSERT INTO fact.FactPlayersTransactional
        SELECT [player_key], [time_key], [competition_key],
               [club_key], [game_key], [type], [minute]
        FROM tmp.FactPlayersTransactional_final;
	SET @number = @@ROWCOUNT;
    INSERT INTO logg.logg VALUES('Insert', 'FactPlayersTransactional',@number,@startdate, GETDATE(), 'insert to table from tmpFactPlayersTransactional_final');
END;
